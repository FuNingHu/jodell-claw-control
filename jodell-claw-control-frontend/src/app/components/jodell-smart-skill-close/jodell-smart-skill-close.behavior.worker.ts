...
import { JodellSmartSkillCloseInstance } from './jodell-smart-skill-close.node';
import { JodellClawAppNode } from '../jodell-claw-app/jodell-claw-app.node';

const createSmartNode =(): OptionalPromise<JodellSmartSkillCloseInstance> =>({
    type: 'universal-robots-contribution-jodell-smart-skill-close',
    name: 'Jodell Close',
    enabled: true,
    parameters:{
    }
...
const behaviors: SmartSkillBehaviors = {
    ...
    startExecution: async (instance : JodellSmartSkillCloseInstance) => {
        ...
        builder.end();
        builder.sleep(0.2);
        builder.addStatements('jodell_daemon.doCmd(0,100,100)'); //(pos, speed, torque)
        return builder;
    },
